...
(define get-procedure-by-arity
       (lambda (proc-name rands env)
         (let ((proc (eval-expression  proc-name   env))
               )
           (if  (= (length rands) (arity proc))
               proc
              (get-procedure-by-arity proc-name rands (env->get-parent env))
...
(define arity
  (lambda (proc)
    (if (procval? proc)
      (length (proc->get-rands proc))
       (eopl:error 'arity
                                 "El primer valor encontrado para el identificador dado no es un procedimiento, tiene el valor de: ~s" proc)))
      )
...
;;;;PROCEDIMIENTOS;;;;
;;Retorna la lista de argumentos que recibe un procedimiento
(define proc->get-rands
  (lambda (proc)
    (cases procval proc
      (closure (ids body env)
               ids)
      (else
       (eopl:error 'proc->get-rands
                                 "Debe ingresar un procdimiento, ha ingresado: " proc)
       )
      )
    )
  )
...
;;;;;AMBIENTES;;;;
;;Retorna el procedimiento padre de un ambiente

(define env->get-parent
  (lambda (env)
    (cases environment env
      (extended-env-record (syms vals env) env)
      (recursively-extended-env-record (proc-names idss bodies env) env)
      (empty-env-record () (eopl:error 'env->get-parent
                                 "El ambiente vacio no tiene padres" env))
